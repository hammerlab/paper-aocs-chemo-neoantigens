...
We identified 18,336 mutated peptides predicted to bind autologous MHC class I with affinity 500nm or tighter ~\cite{Sette1994}, which we refer to as neoantigens. All but 26 (0.14\%) neoantigens were private to a single patient.

In a stratified analysis of all samples irrespective of donor, solid tissue samples taken after adjuvant chemotherapy and disease recurrence harbored more mutations, neoantigens, and expressed neoantigens than chemotherapy-naive primary solid tissue samples (Mann-Whitney $p < 0.004$ for all tests, Figure~\ref{fig:counts}). Similar trends were observed for ascites samples ($p = 0.08, 0.11, 0.04$ for increases in mutation, neoantigen, and expressed neoantigen load, respectively), and in a pairwise comparison of samples from the same donors (Figure~\ref{fig:supp_paired}).

In contrast, neoadjuvant-treated primary samples, which better isolate any effect of chemotherapy because disease recurrence is not a factor, did not show an increase in mutations, neoantigens, or expressed neoantigens, and in fact showed a trend toward decreased burden. In particular, solid-tissue samples harbored a median of 16 expressed neoantigens compared to the median of 44 observed in chemotherapy naive samples ($p=0.09$), an effect which appeared to be due to both fewer neoantigens in the genome (85 vs. 130, $p=0.39$), and a lower rate of expression (19 vs. 39 percent of neoantigens expressed, $p=0.007$).

Similar to results originally reported by Patch et al, the top mutational signatures were COSMIC \textit{Signature 3}, associated with BRCA disruption, \textit{Signature 8}, of unknown etiology, and \textit{Signature 1}, associated with a slow mutagenic process active in healthy tissue. These signatures together accounted for over half of the mutations and neoantigens in both treated and untreated samples.
