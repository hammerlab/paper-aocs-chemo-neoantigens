...
\caption{\textbf{Mutational signature deconvolutions for donors with paired pre- and post-chemo samples.} (Top) Mutational signatures detected in the pre-treatment samples. (Bottom left) Clinical treatments and (bottom right) signature deconvolutions for the post-treatment mutations with no evidence in the pre-treatment samples. The leftmost mutational signatures were extracted from reports of mutations in a \textit{G. gallus}\cite{Szikriszt_2016} cell line and \textit{C. Elegans}\cite{Meier_2014} organisms exposed to chemotherapies. The remaining signatures are from the COSMIC mutational signatures database\cite{364242}. The number in parenthesis indicates the COSMIC signature number; the signature's association is indicated when known. For the chemotherapy signatures, each cell where the signature is detected is annotated according to its agreement with the donor's clinical record. A (*) indicates the donor received the drug associated with the signature, a (+) indicates the donor received a drug in the same family (e.g. platinum compounds), and a '?' indicates there is no record of the donor receiving a similar drug. Signatures not shown received 0 attribution in these samples.}
