...
\caption{\textbf{Sample counts and mean mutations, neoantigens, and expressed noeantigens per sample.} The model-adjusted effect is calculated using a Bayesian model that controls for sample purity and the number of other samples from the same donor, but does not separate effect of treatment from non-treatment effects such as drift due to the later time-point of recurrence samples. The model effects are shown with 95\% credible regions, and the means with bootstrap-estimated 95\% confidence intervals of the error of the mean.}
\label{tab:cohort}
\end{table}
